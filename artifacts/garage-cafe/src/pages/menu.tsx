...
function AddButton({ onAdd, onInc, onDec, qty }: { onAdd: () => void; onInc: () => void; onDec: () => void; qty: number }) {
  if (qty === 0) {
    return (
      <button
        onClick={onAdd}
        className="flex-shrink-0 flex items-center gap-1 bg-[#D97706]/15 hover:bg-[#D97706]/30 border border-[#D97706]/40 hover:border-[#D97706] text-[#D97706] font-['Montserrat'] font-semibold text-[10px] tracking-wider uppercase px-2.5 py-1.5 rounded-lg transition-all duration-200 hover:scale-105"
      >
  ...
  return (
    <div className="flex-shrink-0 flex items-center gap-1.5 bg-[#D97706]/15 border border-[#D97706]/40 rounded-lg px-1.5 py-1">
      <button onClick={onDec} className="w-5 h-5 rounded flex items-center justify-center text-[#D97706] hover:bg-[#D97706]/20 transition-colors font-bold text-sm">−</button>
      <span className="text-white font-['Montserrat'] font-semibold text-xs w-4 text-center">{qty}</span>
      <button onClick={onInc} className="w-5 h-5 rounded flex items-center justify-center bg-[#D97706] hover:bg-[#B87333] text-white transition-colors font-bold text-sm">+</button>
    </div>
  );
}
...
function SimpleItemCard({ name, category }: { name: string; category: string }) {
  ...
  const qty = cartItem?.qty ?? 0;
  return (
    <div className="flex items-center gap-3 p-3.5 bg-white/5 border border-white/8 hover:border-[#D97706]/40 rounded-xl transition-all duration-300 hover:-translate-y-0.5 group">
