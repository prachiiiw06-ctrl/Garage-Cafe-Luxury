...
function ProductCard({ product }: { product: MerchItem }) {
  ...
  return (
    <div className="group bg-white/5 border border-white/8 hover:border-[#B87333]/40 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#B87333]/10">
      ...
      <div className="p-4">
        <span className="text-[#B87333] text-[10px] font-['Montserrat'] font-semibold tracking-[0.2em] uppercase">{product.category}</span>
        <h4 className="font-['Playfair_Display'] text-white font-bold text-base leading-snug mt-1">{product.name}</h4>
        <div className="flex items-center justify-between mt-3">
          <span className="font-['Bebas_Neue'] text-2xl text-[#D97706] tracking-wide">₹{product.price}</span>
          {qty === 0 ? (
            <button
              onClick={() => addItem(product)}
              className="flex items-center gap-1.5 bg-[#B87333]/15 hover:bg-[#B87333]/30 border border-[#B87333]/40 hover:border-[#B87333] text-[#B87333] font-['Montserrat'] font-semibold text-xs tracking-wider uppercase px-4 py-2 rounded-full transition-all duration-200 hover:scale-105"
            >
            ...
            <div className="flex items-center gap-2 bg-[#B87333]/15 border border-[#B87333]/40 rounded-full px-2 py-1.5">
              <button onClick={() => updateQty(product.id, -1)} className="w-5 h-5 rounded-full flex items-center justify-center text-[#B87333] hover:bg-[#B87333]/20 transition-colors font-bold text-sm">−</button>
              <span className="text-white font-['Montserrat'] font-semibold text-xs w-4 text-center">{qty}</span>
              <button onClick={() => updateQty(product.id, 1)} className="w-5 h-5 rounded-full flex items-center justify-center bg-[#B87333] hover:bg-[#a05e29] text-white transition-colors font-bold text-sm">+</button>
            </div>
          )}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-[#111111] font-['Poppins'] pt-20">
      ...
      <div className="relative py-16 px-4 overflow-hidden">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <FadeUp>
            <span className="text-[#B87333] font-['Montserrat'] font-semibold text-xs tracking-[0.4em] uppercase">Garage Merch</span>
            <h1 className="font-['Bebas_Neue'] text-6xl sm:text-8xl text-white mt-3 tracking-wider">
              THE GARAGE <span className="text-[#B87333]">SHOP</span>
            </h1>
            <div className="w-20 h-0.5 bg-[#B87333] mx-auto mt-4" />
            ...
            {totalCount > 0 && (
              <button
                onClick={openCart}
                className="mt-6 inline-flex items-center gap-2 border-2 border-[#B87333] text-[#B87333] hover:bg-[#B87333] hover:text-white font-['Montserrat'] font-semibold tracking-wider uppercase px-6 py-2.5 rounded-full transition-all duration-300 hover:scale-105 text-sm"
              >
                View Bag ({totalCount})
              </button>
            )}
