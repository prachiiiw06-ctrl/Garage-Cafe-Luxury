    ...
    <div className="min-h-screen bg-[#111111] text-white overflow-x-hidden">
      ...
      >
          ...
          <div className="flex items-center justify-between h-16 lg:h-20">
            ...
            <Link href="/">
              <div className="flex items-center gap-3 cursor-pointer group">
                <div className="w-10 h-10 rounded-full border-2 border-[#D97706] flex items-center justify-center">
                ...
                <div>
                  <div className="font-['Bebas_Neue'] text-xl tracking-widest text-white leading-none group-hover:text-[#D97706] transition-colors">
                    THE GARAGE
                  </div>
                  <div className="text-[#B87333] text-[10px] tracking-[0.3em] uppercase font-['Montserrat'] font-semibold leading-none">
                    CAFE
                  </div>
                </div>
              </div>
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href}>
                  <span
                    className={`font-['Montserrat'] font-semibold text-sm tracking-widest uppercase cursor-pointer transition-colors hover:text-[#D97706] ${
                      location === link.href ? "text-[#D97706]" : "text-white/80"
                    }`}
                  >
                    {link.label}
                  </span>
        ...
        {menuOpen && (
          <div className="md:hidden bg-[#111111]/98 backdrop-blur-md border-t border-[#ffffff10] px-4 py-6">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <div className="py-3 font-['Montserrat'] font-semibold tracking-widest uppercase text-white/80 hover:text-[#D97706] transition-colors border-b border-[#ffffff08] cursor-pointer">
                  {link.label}
                </div>
              </Link>
            ))}
      ...
      {showTop && (
        <button
          ...
          className="fixed bottom-24 right-5 z-40 w-11 h-11 bg-[#D97706] hover:bg-[#B87333] rounded-full flex items-center justify-center shadow-lg shadow-[#D97706]/40 transition-all hover:scale-110"
        >
